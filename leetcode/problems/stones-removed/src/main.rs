...
pub fn test(input: Vec<(i32, i32)>, expected: i32) {
    let input: Vec<Vec<i32>> = input.iter().map(|t| vec![t.0, t.1]).collect();
    assert_eq!(remove_stones(input), expected);
}

#[test]
fn test_1() {
    test(vec![(0,0),(0,1),(1,0),(1,2),(2,1),(2,2)], 5);
}
